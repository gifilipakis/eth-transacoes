...
contract Transacoes {
    ...
    function criarTransacao(string memory _comprador, string memory _produto, string memory _unidade, 
    uint _preco, uint _quantidade, string memory _info_adicionais) public {
        qntdTransacoes ++;
        transacoes[qntdTransacoes] = Transacao(qntdTransacoes, _comprador, _produto, _unidade, _preco, _quantidade, _info_adicionais, false);
        emit TransacaoCriada(qntdTransacoes, _comprador, _produto, _unidade, _preco, _quantidade, _info_adicionais, false);
    }
